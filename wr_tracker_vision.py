...
SOFT_SPEED_THRESHOLD_MPH = 18.25
MAX_REASONABLE_SPEED_MPH = 20.75
SIDE_VIEW_SPEED_ADJUSTMENT = 0.95
FAR_CLIP_BOX_HEIGHT_PX = 85.0
MID_CLIP_BOX_HEIGHT_PX = 120.0
...
KINEMATICS_POSITION_WINDOW = 11
KINEMATICS_DERIVATIVE_HALF_WINDOW = 4
OFF_LINE_START_SPEED_MPH = 4.0
OFF_LINE_START_CONFIRM_FRAMES = 4
OFF_LINE_START_BACKTRACK_FRAMES = 2
OFF_LINE_START_MIN_FWD_DIST_YD = 0.55
OFF_LINE_RESET_SPEED_MPH = 2.0
OFF_LINE_MAX_BACKTRACK_FRAMES = 18
PREBREAK_ROUTE_SPEED_CAPS_MPH = {
    "Curl": 12.0,
    "Slant": 12.0,
}
...
def estimate_sideview_field_line_scale(
    frame: np.ndarray,
    homography: np.ndarray,
) -> tuple[float | None, float, dict]:
    """
    Estimate a modest field-scale correction from visible field-line structure.

    For side-view clips, yard lines often appear as a family of roughly vertical
    bright segments. If we can detect several of those lines with consistent
    spacing, we can compare the projected world spacing to the expected 5-yard
    spacing and gently correct the global scale.
    """
    info = {
        "line_clusters": 0,
        "median_gap_yards": None,
    }

    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    turf_mask = cv2.inRange(hsv, np.array([25, 25, 25], dtype=np.uint8), np.array([95, 255, 255], dtype=np.uint8))
    turf_mask = cv2.morphologyEx(turf_mask, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8), iterations=1)

    white_mask = cv2.inRange(hsv, np.array([0, 0, 155], dtype=np.uint8), np.array([180, 90, 255], dtype=np.uint8))
    white_mask = cv2.bitwise_and(white_mask, turf_mask)
    ...
    lines = cv2.HoughLinesP(
        ...
        theta=np.pi / 180.0,
        threshold=28,
        minLineLength=max(40, int(frame.shape[0] * 0.14)),
        maxLineGap=18,
    )
    if lines is None:
        return None, 0.0, info

    vertical_midpoints: list[tuple[float, float, float]] = []
    for line in lines[:, 0, :]:
        x1, y1, x2, y2 = [float(v) for v in line]
        dx = x2 - x1
        dy = y2 - y1
        length = math.hypot(dx, dy)
        if length < max(40.0, frame.shape[0] * 0.14):
            continue
        angle = abs(math.degrees(math.atan2(dy, dx)))
        if angle < 60.0 or angle > 120.0:
            continue
        vertical_midpoints.append(((x1 + x2) * 0.5, (y1 + y2) * 0.5, length))

    if len(vertical_midpoints) < 3:
        return None, 0.0, info

    vertical_midpoints.sort(key=lambda item: item[0])
    clustered_xs: list[tuple[float, float]] = []
    cluster_tol = max(12.0, frame.shape[1] * 0.012)
    current_cluster: list[tuple[float, float, float]] = [vertical_midpoints[0]]
    for point in vertical_midpoints[1:]:
        if abs(point[0] - current_cluster[-1][0]) <= cluster_tol:
            current_cluster.append(point)
        else:
            xs = [p[0] for p in current_cluster]
            ys = [p[1] for p in current_cluster]
            clustered_xs.append((float(np.median(xs)), float(np.median(ys))))
    ...
    info["line_clusters"] = len(clustered_xs)
    if len(clustered_xs) < 3:
        return None, 0.0, info

    clustered_xs.sort(key=lambda item: item[0])
    world_xs: list[float] = []
    for x_img, y_img in clustered_xs:
        xw, _ = image_to_world((x_img, y_img), homography)
        world_xs.append(float(xw))

    gaps = np.diff(np.array(world_xs, dtype=float))
    gaps = np.abs(gaps)
    gaps = gaps[(gaps > 1.0) & (gaps < 12.0)]
    if len(gaps) < 2:
        return None, 0.0, info

    median_gap = float(np.median(gaps))
    gap_cv = float(np.std(gaps) / max(np.mean(gaps), 1e-6))
    info["median_gap_yards"] = round(median_gap, 3)

    # Assume adjacent major field lines are usually 5 yards apart.
    scale_factor = 5.0 / max(median_gap, 1e-6)
    scale_factor = max(0.84, min(1.28, scale_factor))
    confidence = max(0.0, min(0.8, 0.25 + 0.15 * min(len(gaps), 5) - 0.35 * gap_cv))
    if confidence < 0.25:
        return None, confidence, info
    return scale_factor, confidence, info


def resolve_field_scale_factor(
    df: pd.DataFrame,
    scale_factor: float | None,
    confidence: float | None,
) -> float | None:
    if df.empty or scale_factor is None:
        return scale_factor

    effective = float(scale_factor)
    conf = float(confidence or 0.0)
    avg_box_height = None
    if "box_height_px" in df.columns:
        box_h = df["box_height_px"].to_numpy(dtype=float)
        box_h = box_h[np.isfinite(box_h) & (box_h > 1.0)]
        if len(box_h):
            avg_box_height = float(np.median(box_h))

    max_scale = 1.28
    min_scale = 0.84
    if avg_box_height is not None:
        if avg_box_height >= 235.0:
            max_scale = 1.14
            min_scale = 0.90
        elif avg_box_height >= 205.0:
            max_scale = 1.20
            min_scale = 0.88
        elif avg_box_height >= 170.0:
            max_scale = 1.28
            min_scale = 0.86

    if effective > 1.0 and conf >= 0.6 and avg_box_height is not None and avg_box_height < 205.0:
        # Give smaller/farther subjects a little more room without heating up close clips.
        effective *= 1.03

    return max(min_scale, min(max_scale, effective))


def apply_field_scale_correction(df: pd.DataFrame, scale_factor: float | None) -> pd.DataFrame:
    if df.empty or scale_factor is None:
        return df
    work = df.copy()
    for col in ("field_x_yd", "field_y_yd"):
        if col in work.columns:
            work[col] = np.round(work[col].to_numpy(dtype=float) * float(scale_factor), 3)
    return work
...
def recompute_frame_kinematics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if n >= 5:
        speed_yps = smooth_series(speed_yps, window=9)
    ...
    if n >= 5:
        accel_yps2 = smooth_series(accel_yps2, window=9)
...
def apply_speed_adjustment(
    ...
    analysis_mode: str,
    field_scale_factor: float | None = None,
) -> pd.DataFrame:
    if df.empty or analysis_mode != "side-view":
        return df

    work = df.copy()
    speed_profile = infer_speed_profile(work, calibration_confidence, analysis_mode, field_scale_factor=field_scale_factor)
    distance_profile = speed_profile.get("distance_profile")
    avg_box_height = speed_profile.get("avg_box_height_px")
    confidence = 0.75 if calibration_confidence is None else max(0.0, min(1.0, float(calibration_confidence)))
    confidence_factor = 0.95 + 0.05 * confidence
    profile_factor = 1.0
    soft_threshold = 16.5
    hard_max = MAX_REASONABLE_SPEED_MPH
    route_distance = None
    if "field_x_yd" in work.columns and "field_y_yd" in work.columns and len(work) >= 2:
        x_vals = work["field_x_yd"].to_numpy(dtype=float)
        y_vals = work["field_y_yd"].to_numpy(dtype=float)
        step_dist = np.sqrt(np.diff(x_vals) ** 2 + np.diff(y_vals) ** 2)
        route_distance = float(np.nansum(step_dist))
    if distance_profile == "far":
        profile_factor = 1.03
        soft_threshold = min(MAX_REASONABLE_SPEED_MPH, 17.5)
    elif distance_profile == "medium":
        profile_factor = 1.01
        soft_threshold = min(MAX_REASONABLE_SPEED_MPH, 17.0)
    elif route_distance is not None and route_distance <= 9.0:
        profile_factor = 0.92
        soft_threshold = min(MAX_REASONABLE_SPEED_MPH, 15.25)
        if route_distance <= 7.0:
            profile_factor = 0.9
            soft_threshold = min(MAX_REASONABLE_SPEED_MPH, 14.9)
    elif distance_profile == "close" and confidence < 0.72:
        profile_factor = 0.93
        soft_threshold = min(MAX_REASONABLE_SPEED_MPH, 15.85)

    # Tight close-up clips without any field-line support can look deceptively
    # fast because the homography scale is driven mostly by the player's local
    # motion. Cool those more aggressively, but only when the crop is truly
    # large on screen so we do not drag down healthier close clips like Clip2OR.
    if (
        distance_profile == "close"
        and (field_scale_factor is None or not np.isfinite(field_scale_factor))
        and avg_box_height is not None
        and float(avg_box_height) >= 270.0
    ):
        profile_factor *= 0.88
        soft_threshold = min(soft_threshold, 14.8)
        if float(avg_box_height) >= 295.0:
            profile_factor *= 0.96
            soft_threshold = min(soft_threshold, 14.5)
    factor = SIDE_VIEW_SPEED_ADJUSTMENT * confidence_factor * profile_factor

    for col in ("speed_yards_per_sec", "speed_mph", "accel_yards_per_sec2", "accel_mph_per_sec"):
        if col in work.columns:
            work[col] = np.round(work[col].to_numpy(dtype=float) * factor, 3)

    if "frame" in work.columns and "speed_mph" in work.columns:
        base_speed = work["speed_mph"].to_numpy(dtype=float)
        speed = base_speed.copy()
        speed = smooth_series(smooth_series(speed, window=7 if distance_profile == "close" else 9), window=9 if distance_profile == "close" else 11)
        # Only compress suspicious top-end spikes; leave normal route speed mostly intact.
        compressed_speed: list[float] = []
        span = max(hard_max - soft_threshold, 1e-6)
        for value in speed:
            if value <= soft_threshold:
                compressed_speed.append(float(value))
                continue
            excess = float(value - soft_threshold)
            compressed_value = soft_threshold + span * (1.0 - math.exp(-excess / span))
            compressed_speed.append(min(compressed_value, hard_max))
        speed = np.array(compressed_speed, dtype=float)

        # If the player is getting smaller on screen, they are likely moving away
        # from the camera. That can make side-view homography speed read too hot,
        # especially after the break, so gently damp those frames.
        if "box_height_px" in work.columns:
            box_h = work["box_height_px"].to_numpy(dtype=float)
            box_h = smooth_series(box_h, window=7)
            scale = np.ones(len(box_h), dtype=float)
            running_max = 0.0
            valid_box_h = box_h[np.isfinite(box_h) & (box_h > 1.0)]
            clip_median = float(np.median(valid_box_h)) if len(valid_box_h) else None
            for idx in range(len(box_h)):
                lo = max(0, idx - 10)
                recent = box_h[lo:idx + 1]
                recent = recent[np.isfinite(recent) & (recent > 1.0)]
                if len(recent) < 4:
                    continue
                running_max = max(running_max, float(np.max(recent)))
                baseline = float(np.percentile(recent, 75))
                current = float(box_h[idx])
                if not np.isfinite(current) or current <= 1.0 or baseline <= 1.0 or running_max <= 1.0:
                    continue
                recent_ratio = current / baseline
                global_ratio = current / running_max
                ratio = min(recent_ratio, global_ratio)
                median_ratio = None
                if clip_median is not None and clip_median > 1.0:
                    median_ratio = current / clip_median

                # Continuous frame-by-frame correction:
                # smaller on screen than the clip median -> modest boost
                # larger on screen than the clip median -> modest damping
                # This lets a single clip move gradually from farther to closer
                # without forcing the whole rep into one distance bucket.
                perspective_scale = 1.0
                if median_ratio is not None:
                    if median_ratio < 0.96:
                        if distance_profile == "far":
                            perspective_scale *= min(1.18, max(1.0, median_ratio ** -0.28))
                        elif distance_profile == "medium":
                            perspective_scale *= min(1.14, max(1.0, median_ratio ** -0.22))
                        else:
                            perspective_scale *= min(1.1, max(1.0, median_ratio ** -0.16))
                    elif median_ratio > 1.06:
                        if distance_profile == "far":
                            perspective_scale *= max(0.92, median_ratio ** -0.18)
                        elif distance_profile == "medium":
                            perspective_scale *= max(0.89, median_ratio ** -0.24)
                        else:
                            perspective_scale *= max(0.85, median_ratio ** -0.32)

                # Keep the away-from-camera guard, but make it secondary to the
                # continuous median-based perspective scale above.
                if ratio < 0.9:
                    if distance_profile == "far":
                        perspective_scale *= max(0.92, ratio ** 0.12)
                    elif distance_profile == "medium":
                        perspective_scale *= max(0.88, ratio ** 0.2)
                    else:
                        perspective_scale *= max(0.82, ratio ** 0.35)
                elif ratio > 1.02:
                    if distance_profile == "far":
                        perspective_scale *= max(0.95, ratio ** -0.08)
                    elif distance_profile == "medium":
                        perspective_scale *= max(0.92, ratio ** -0.12)
                    else:
                        perspective_scale *= max(0.88, ratio ** -0.18)

                scale[idx] = perspective_scale
            speed = speed * scale

        # Far-away clips benefit from some local route-shape being preserved so
        # the break slowdown does not get flattened completely by the global
        # smoothing profile.
        local_window = 3 if distance_profile == "far" else 5
        local_speed = smooth_series(base_speed, window=local_window)
        if distance_profile == "far":
            speed = 0.7 * speed + 0.3 * local_speed
        elif distance_profile == "medium":
            speed = 0.82 * speed + 0.18 * local_speed

        work["speed_mph"] = np.round(speed, 3)
        work["speed_yards_per_sec"] = np.round(speed * 1760.0 / 3600.0, 3)

    if "accel_mph_per_sec" in work.columns:
        accel = smooth_series(work["accel_mph_per_sec"].to_numpy(dtype=float), window=7)
        work["accel_mph_per_sec"] = np.round(accel, 3)
        work["accel_yards_per_sec2"] = np.round(accel * 1760.0 / 3600.0, 3)
    return work
...
def infer_speed_profile(
    ...
    profile = {
        "distance_profile": "unknown",
        "avg_box_height_px": None,
        "speed_confidence": "medium" if analysis_mode == "side-view" else "high",
        "speed_confidence_detail": None,
    }
    if df.empty or analysis_mode != "side-view" or "box_height_px" not in df.columns:
        return profile

    box_h = df["box_height_px"].to_numpy(dtype=float)
    box_h = box_h[np.isfinite(box_h) & (box_h > 1.0)]
    if len(box_h) == 0:
        profile["speed_confidence"] = "low"
        profile["speed_confidence_detail"] = "Player size on screen was not stable enough to judge absolute mph confidently."
        return profile

    avg_box_height = float(np.median(box_h))
    profile["avg_box_height_px"] = round(avg_box_height, 3)
    if avg_box_height < FAR_CLIP_BOX_HEIGHT_PX:
        profile["distance_profile"] = "far"
    elif avg_box_height < MID_CLIP_BOX_HEIGHT_PX:
        profile["distance_profile"] = "medium"
    else:
        profile["distance_profile"] = "close"

    calibration = 0.75 if calibration_confidence is None else max(0.0, min(1.0, float(calibration_confidence)))
    speed_values = df["speed_mph"].to_numpy(dtype=float) if "speed_mph" in df.columns else np.array([], dtype=float)
    speed_values = speed_values[np.isfinite(speed_values)]
    peak_speed = float(np.percentile(speed_values, 95)) if len(speed_values) else 0.0
    avg_speed = float(np.mean(speed_values)) if len(speed_values) else 0.0
    speed_range = float(np.max(speed_values) - np.min(speed_values)) if len(speed_values) else 0.0
    route_distance = None
    if "field_x_yd" in df.columns and "field_y_yd" in df.columns and len(df) >= 2:
        x_vals = df["field_x_yd"].to_numpy(dtype=float)
        y_vals = df["field_y_yd"].to_numpy(dtype=float)
        step_dist = np.sqrt(np.diff(x_vals) ** 2 + np.diff(y_vals) ** 2)
        route_distance = float(np.nansum(step_dist))

    if profile["distance_profile"] == "close":
        if calibration >= 0.85 and peak_speed >= 15.0 and speed_range >= 6.0:
            profile["speed_confidence"] = "high"
        elif peak_speed < 13.5 or speed_range < 4.0:
            profile["speed_confidence"] = "low"
        else:
            profile["speed_confidence"] = "medium"
    elif profile["distance_profile"] == "medium":
        profile["speed_confidence"] = "medium" if calibration >= 0.55 else "low"
    else:
        profile["speed_confidence"] = "low"

    if profile["distance_profile"] == "far":
        profile["speed_confidence_detail"] = "This clip is farther away, so the speed curve is more useful than the exact mph number."
    elif profile["distance_profile"] == "medium":
        profile["speed_confidence_detail"] = "Speed is usable for comparison, but exact mph can drift on medium-distance clips."
    elif profile["speed_confidence"] == "low":
        profile["speed_confidence_detail"] = "The route read looks usable, but the absolute mph on this clip does not look strong enough to treat as a precise number."
    elif profile["speed_confidence"] == "medium":
        profile["speed_confidence_detail"] = "Speed looks usable, but field scale confidence is not strong enough to treat mph as exact."

    if route_distance is not None and route_distance <= 9.0 and peak_speed >= 16.0:
        if profile["speed_confidence"] == "high":
            profile["speed_confidence"] = "medium"
        profile["speed_confidence_detail"] = "This is a shorter route, so small tracking and scale errors can make the top mph and burst look hotter than they really are."

    if (
        profile["distance_profile"] == "close"
        and (field_scale_factor is None or not np.isfinite(field_scale_factor))
        and avg_box_height >= 270.0
    ):
        if profile["speed_confidence"] == "high":
            profile["speed_confidence"] = "medium"
        elif profile["speed_confidence"] == "medium":
            profile["speed_confidence"] = "low"
        profile["speed_confidence_detail"] = "This is a tight close-up without strong yard-marker scale support, so the speed curve is more trustworthy than the exact mph."

    return profile


def preview_speed_display_values(
    speed_mph: float,
    ...
    if analysis_mode != "side-view":
        return float(speed_mph), float(accel_mph_per_sec)

    confidence = 0.75 if calibration_confidence is None else max(0.0, min(1.0, float(calibration_confidence)))
    confidence_factor = 0.95 + 0.05 * confidence
    factor = SIDE_VIEW_SPEED_ADJUSTMENT * confidence_factor

    display_speed = float(speed_mph) * factor
    display_accel = float(accel_mph_per_sec) * factor

    soft_threshold = 16.5
    hard_max = MAX_REASONABLE_SPEED_MPH
    if display_speed > soft_threshold:
        span = max(hard_max - soft_threshold, 1e-6)
        excess = display_speed - soft_threshold
        display_speed = min(soft_threshold + span * (1.0 - math.exp(-excess / span)), hard_max)

    return display_speed, display_accel
...
def compute_route_phase_metrics(
    ...
    speed_profile = infer_speed_profile(df, None, "side-view") if "box_height_px" in df.columns else {"distance_profile": "unknown"}
    distance_profile = speed_profile.get("distance_profile")
    route_window = 5 if distance_profile == "close" else 3
    speed_window = 5 if distance_profile == "close" else 3
    speed_score_weight = 1.0
    drop_support_scale = 1.0
    if distance_profile == "far":
        speed_score_weight = 0.45
        drop_support_scale = 0.45
    elif distance_profile == "medium":
        speed_score_weight = 0.7
        drop_support_scale = 0.7

    x = smooth_series(df["field_x_yd"].to_numpy(dtype=float), window=route_window)
    y = smooth_series(df["field_y_yd"].to_numpy(dtype=float), window=route_window)
    t = df["time_s"].to_numpy(dtype=float)
    speed_mph = smooth_series(df["speed_mph"].to_numpy(dtype=float), window=speed_window)
    local_speed_mph = smooth_series(df["speed_mph"].to_numpy(dtype=float), window=3)
    ...
    release_start_idx = 0
    early_scan_end = min(len(df) - OFF_LINE_START_CONFIRM_FRAMES, max(10, len(df) // 3))
    for idx in range(max(0, early_scan_end + 1)):
        window_end = min(len(df), idx + OFF_LINE_START_CONFIRM_FRAMES)
        speed_window = local_speed_mph[idx:window_end]
        if len(speed_window) < OFF_LINE_START_CONFIRM_FRAMES:
            continue
        fwd_dist = cumulative_dist[min(len(cumulative_dist) - 1, window_end - 1)] - cumulative_dist[idx]
        if float(np.median(speed_window)) >= OFF_LINE_START_SPEED_MPH and fwd_dist >= OFF_LINE_START_MIN_FWD_DIST_YD:
            release_start_idx = max(0, idx - OFF_LINE_START_BACKTRACK_FRAMES)
            break

    # If the trigger happens late, walk back toward the actual release instead of
    # starting burst timing when the player is already at speed. We only walk back
    # a modest distance and stop once the athlete looks close to a true reset.
    backtrack_floor = max(0, release_start_idx - OFF_LINE_MAX_BACKTRACK_FRAMES)
    for idx in range(release_start_idx, backtrack_floor - 1, -1):
        if local_speed_mph[idx] <= OFF_LINE_RESET_SPEED_MPH:
    ...
    cut = {
        ...
        "cut_speed_drop_mph": None,
        "cut_speed_drop_pct": None,
        "brake_start_frame": None,
        "brake_end_frame": None,
        "brake_entry_speed_mph": None,
        "brake_low_speed_mph": None,
        "brake_decel_mph_per_sec": None,
        "brake_speed_drop_mph": None,
        "brake_speed_drop_pct": None,
    ...
    if len(points) >= 16:
        ...
        for idx in range(start_idx, end_idx):
            ...
            local_speed_window = local_speed_mph[max(0, idx - 4): min(len(local_speed_mph), idx + 5)]
            ...
            if pre_span < 1.5 or post_span < 1.5:
                continue
            stem_progress = float(cumulative_dist[idx] - cumulative_dist[0])
            entry_speed = float(np.max(local_speed_mph[max(0, idx - 4): idx + 1]))
            exit_speed = float(np.min(local_speed_mph[idx: min(len(local_speed_mph), idx + 5)]))
            speed_drop = max(0.0, entry_speed - exit_speed)
            speed_factor = 1.0 + max(float(local_speed_window.max()) - 1.0, 0.0) * speed_score_weight
            early_bias = 1.0 / (1.0 + idx * 0.015)
            if stem_progress >= 8.0:
                early_bias = max(0.82, early_bias)
            if stem_progress >= 11.5:
                early_bias = max(0.9, early_bias)
            angle_support = 1.0
            ...
            candidate_stem_vec = pre_points[-1] - pre_points[0]
            candidate_break_vec = post_points[-1] - post_points[0]
            candidate_stem_norm = float(np.linalg.norm(candidate_stem_vec))
            candidate_break_norm = float(np.linalg.norm(candidate_break_vec))
            normalized_break_along = 0.0
            normalized_break_cross = 0.0
            candidate_full_turn = full_turn_angle_between_deg(candidate_stem_vec, candidate_break_vec)
            candidate_break_field_direction = "unknown"
            if candidate_stem_norm > 1e-6 and candidate_break_norm > 1e-6:
                candidate_stem_unit = candidate_stem_vec / candidate_stem_norm
                candidate_break_unit = candidate_break_vec / candidate_break_norm
                normalized_break_along = float(np.dot(candidate_break_unit, candidate_stem_unit))
                normalized_break_cross = abs(float(candidate_stem_unit[0] * candidate_break_unit[1] - candidate_stem_unit[1] * candidate_break_unit[0]))

                candidate_stem_axis_idx = 0 if abs(candidate_stem_vec[0]) >= abs(candidate_stem_vec[1]) else 1
                candidate_cross_axis_idx = 1 - candidate_stem_axis_idx
                clip_cross_center = float(np.median(points[:, candidate_cross_axis_idx]))
                break_cross_start = float(post_points[0][candidate_cross_axis_idx])
                break_cross_end = float(post_points[-1][candidate_cross_axis_idx])
                start_offset = abs(break_cross_start - clip_cross_center)
                end_offset = abs(break_cross_end - clip_cross_center)
                if reverse_component <= -0.2:
                    candidate_break_field_direction = "reverse"
                elif end_offset >= start_offset + 0.35:
                    candidate_break_field_direction = "out"
                elif end_offset <= start_offset - 0.35:
                    candidate_break_field_direction = "in"

            route_template_support = 0.0
            if reverse_component <= -0.75 and normalized_break_cross <= 0.4:
                route_template_support += 1.9  # curl-like
            elif reverse_component <= -0.2 and normalized_break_cross >= 0.2:
                route_template_support += 1.35  # comeback-like
            elif (
                idealized_turn >= 75.0
                or 60.0 <= candidate_full_turn <= 120.0
                or (normalized_break_cross >= 0.55 and abs(normalized_break_along) <= 0.55)
            ):
                route_template_support += 1.6  # out/dig family
            elif (
                20.0 <= idealized_turn <= 70.0
                and 20.0 <= actual_turn <= 70.0
                and normalized_break_along > -0.15
                and normalized_break_cross <= 0.85
            ):
                route_template_support += 0.9  # slant family

            if candidate_break_field_direction == "out":
                route_template_support += 1.3
                if candidate_full_turn >= 50.0 or normalized_break_cross >= 0.35:
                    route_template_support += 0.8
            elif candidate_break_field_direction == "in":
                route_template_support += 1.4
                if candidate_full_turn >= 55.0 or normalized_break_cross >= 0.35:
                    route_template_support += 0.9
                if reverse_component > -0.25:
                    route_template_support += 0.4
                if stem_progress >= 8.0:
                    route_template_support += 0.7
                if stem_progress >= 11.5:
                    route_template_support += 0.8
            elif candidate_break_field_direction == "reverse":
                route_template_support += 0.8
                if stem_progress >= 8.0 and candidate_full_turn < 145.0:
                    route_template_support -= 0.45

            # Penalize reverse-looking candidates when the geometry is otherwise
            # more perpendicular than truly reversing. This helps stop a later
            # comeback-like segment from beating the actual in-breaking dig cut.
            if (
                candidate_break_field_direction == "reverse"
                and candidate_full_turn < 150.0
                and normalized_break_cross >= 0.45
                and reverse_component > -0.65
            ):
                route_template_support -= 0.9
            ...
            if reverse_component <= -0.2:
                ...
                angle_support += 0.2
            drop_support = 1.0 + min(speed_drop / 8.0, 0.5) * drop_support_scale
            score = max(turn_deg, actual_turn) * speed_factor * min(pre_span, post_span) * early_bias * angle_support * drop_support * (1.0 + route_template_support)
            candidate = {
                "idx": idx,
                "turn_deg": turn_deg,
                ...
                "candidate_full_turn": candidate_full_turn,
                "candidate_break_field_direction": candidate_break_field_direction,
                "route_template_support": route_template_support,
                "score": score,
                "pre_dir": pre_dir,
                "post_dir": post_dir,
            ...
            significantly_earlier = (
                ...
                and (
                    ...
                    or route_template_support >= best.get("route_template_support", 0.0) + 0.35
                )
            )
            materially_stronger = score > best["score"] * 1.25
            stronger_field_route = (
                candidate_break_field_direction in {"out", "in", "reverse"}
                and best.get("candidate_break_field_direction", "unknown") == "unknown"
                and (
                    route_template_support >= best.get("route_template_support", 0.0) + 0.6
                    or candidate_full_turn >= best.get("candidate_full_turn", 0.0) + 12.0
                )
            )
            better_in_break = (
                candidate_break_field_direction == "in"
                and best.get("candidate_break_field_direction", "unknown") in {"unknown", "reverse"}
                and (
                    route_template_support >= best.get("route_template_support", 0.0) + 0.45
                    or (
                        normalized_break_cross >= 0.45
                        and candidate_full_turn >= 55.0
                        and reverse_component > -0.35
                    )
                )
            )
            later_deep_in_break = (
                candidate_break_field_direction == "in"
                and stem_progress >= 8.0
                and idx >= best["idx"] + 4
                and (
                    candidate_full_turn >= best.get("candidate_full_turn", 0.0) + 18.0
                    or route_template_support >= best.get("route_template_support", 0.0) + 0.8
                )
            )
            if significantly_earlier or materially_stronger or stronger_field_route or better_in_break or later_deep_in_break:
                best = {
                    **candidate,
                }
        ...
        if best is not None:
            ...
            onset_idx = idx
            stem_norm_for_onset = float(np.linalg.norm(stem_vec))
            if stem_norm_for_onset > 1e-6:
                stem_unit_for_onset = stem_vec / stem_norm_for_onset
                onset_scan_start = max(stem_seed_end, idx - 10)
                onset_angle_threshold = max(15.0, best["actual_turn"] * 0.35)
                for candidate_idx in range(onset_scan_start, idx + 1):
                    lookahead_idx = min(len(points) - 1, candidate_idx + 3)
                    if lookahead_idx <= candidate_idx:
                        continue
                    local_vec = points[lookahead_idx] - points[candidate_idx]
                    local_norm = float(np.linalg.norm(local_vec))
                    if local_norm <= 1e-6:
                        continue
                    local_turn = acute_angle_between_deg(stem_unit_for_onset, local_vec / local_norm)
                    rel_point = points[candidate_idx] - points[max(0, onset_scan_start - 2)]
                    cross_track = abs(float(stem_unit_for_onset[0] * rel_point[1] - stem_unit_for_onset[1] * rel_point[0]))
                    if local_turn >= onset_angle_threshold or cross_track >= 0.3:
                        onset_idx = candidate_idx
                        break

            entry_start = max(0, onset_idx - 4)
            exit_end = min(len(df) - 1, onset_idx + 4)
            entry_speed = float(np.max(local_speed_mph[max(0, onset_idx - 4): onset_idx + 1]))
            exit_speed = float(np.min(local_speed_mph[onset_idx: min(len(local_speed_mph), onset_idx + 5)]))
            ...
            if dt > 1e-6:
                ...
                cut["cut_frame"] = int(df["frame"].iloc[onset_idx])
                ...
                cut["cut_speed_drop_mph"] = round(max(0.0, entry_speed - exit_speed), 3)
                if entry_speed > 1e-6:
                    cut["cut_speed_drop_pct"] = round(max(0.0, (entry_speed - exit_speed) / entry_speed) * 100.0, 3)

                # Separate braking from the geometric cut center. Some reps start
                # losing speed slightly before the turn, while others keep carrying
                # speed until a few frames after the first geometric redirect. Scan
                # a tight neighborhood around the break and choose the strongest
                # local speed-loss segment instead of assuming braking is only pre-cut.
                brake_search_start = max(0, onset_idx - 12)
                brake_search_end = min(len(df) - 1, onset_idx + 20)
                best_brake: dict | None = None
                for local_start in range(brake_search_start, brake_search_end):
                    trailing_end = min(brake_search_end + 1, local_start + 17)
                    trailing_window = local_speed_mph[local_start + 1:trailing_end]
                    if len(trailing_window) < 2:
                        continue
                    local_low_rel = int(np.argmin(trailing_window))
                    local_low_idx = local_start + 1 + local_low_rel
                    brake_entry_speed = float(local_speed_mph[local_start])
                    brake_low_speed = float(local_speed_mph[local_low_idx])
                    speed_drop = brake_entry_speed - brake_low_speed
                    if speed_drop < 0.75:
                        continue
                    brake_dt = float(t[local_low_idx] - t[local_start])
                    if brake_dt <= 1e-6:
                        continue
                    decel = (brake_low_speed - brake_entry_speed) / brake_dt
                    onset_distance_penalty = 0.04 * abs(local_start - onset_idx)
                    score = speed_drop + min(abs(decel) / 10.0, 1.5) - onset_distance_penalty
                    if best_brake is None or score > best_brake["score"]:
                        best_brake = {
                            "entry_idx": local_start,
                            "low_idx": local_low_idx,
                            "entry_speed": brake_entry_speed,
                            "low_speed": brake_low_speed,
                            "decel": decel,
                            "drop": speed_drop,
                            "dt": brake_dt,
                            "score": score,
                ...
                if best_brake is not None:
                    brake_entry_idx = int(best_brake["entry_idx"])
                    brake_low_idx = int(best_brake["low_idx"])
                    brake_entry_speed = float(best_brake["entry_speed"])
                    brake_low_speed = float(best_brake["low_speed"])
                    cut["brake_start_frame"] = int(df["frame"].iloc[brake_entry_idx])
                    cut["brake_end_frame"] = int(df["frame"].iloc[brake_low_idx])
                    cut["brake_entry_speed_mph"] = round(brake_entry_speed, 3)
                    cut["brake_low_speed_mph"] = round(brake_low_speed, 3)
                    cut["brake_decel_mph_per_sec"] = round(float(best_brake["decel"]), 3)
                    cut["brake_speed_drop_mph"] = round(max(0.0, brake_entry_speed - brake_low_speed), 3)
                    if brake_entry_speed > 1e-6:
                        cut["brake_speed_drop_pct"] = round(max(0.0, (brake_entry_speed - brake_low_speed) / brake_entry_speed) * 100.0, 3)
                ...
                cut["cut_point_field_x_yd"] = round(float(x[onset_idx]), 3)
                cut["cut_point_field_y_yd"] = round(float(y[onset_idx]), 3)
                ...
                stem_axis_idx = 0 if abs(stem_vec[0]) >= abs(stem_vec[1]) else 1
                cross_axis_idx = 1 - stem_axis_idx
                clip_cross_center = float(np.median(points[:, cross_axis_idx]))
                break_cross_start = float(post_points[0][cross_axis_idx])
                break_cross_end = float(post_points[-1][cross_axis_idx])
                start_offset = abs(break_cross_start - clip_cross_center)
                end_offset = abs(break_cross_end - clip_cross_center)
                if reverse_component <= -0.2:
                    cut["break_field_direction"] = "reverse"
                elif end_offset >= start_offset + 0.35:
                    cut["break_field_direction"] = "out"
                elif end_offset <= start_offset - 0.35:
                    cut["break_field_direction"] = "in"
                else:
                    cut["break_field_direction"] = "unknown"

                if include_pose_metrics and distance_profile != "far" and "hip_mid_y_px" in df.columns:
...
def guess_route_from_phase_metrics(phase_metrics: dict) -> dict:
    ...
    signed_turn_angle = phase_metrics.get("signed_turn_angle_deg")
    ...
    break_field_direction = phase_metrics.get("break_field_direction")
    ...
    stem_length = stem_norm
    break_depth = abs(stem_dx) if stem_axis_idx == 0 else abs(stem_dy)
    deep_break = stem_length >= 8.0 or break_depth >= 8.0
    very_deep_break = stem_length >= 11.5 or break_depth >= 11.5
    post_break_distance = break_norm
    cut_entry_speed = phase_metrics.get("cut_entry_speed_mph")
    cut_exit_speed = phase_metrics.get("cut_exit_speed_mph")
    post_break_reaccel = None
    if cut_entry_speed is not None and cut_exit_speed is not None:
        post_break_reaccel = float(cut_exit_speed) - float(cut_entry_speed)
    ...
    # Reverse-break families.
    if ideal_angle >= 170:
        route_scores["Curl"] += 2.2
    if full_turn_angle >= 155:
        route_scores["Curl"] += 2.0
    if normalized_break_along <= -0.8:
        route_scores["Curl"] += 1.8
    if normalized_break_cross <= 0.4:
        route_scores["Curl"] += 0.8
    if post_break_distance <= 3.0:
        route_scores["Curl"] += 1.2
    elif post_break_distance <= 4.5:
        route_scores["Curl"] += 0.6
    if post_break_reaccel is not None and post_break_reaccel <= 0.75:
        route_scores["Curl"] += 1.0

    if ideal_angle >= 125:
        route_scores["Comeback"] += 1.8
    if full_turn_angle >= 110:
        route_scores["Comeback"] += 1.5
    if reverse_component <= -0.2:
        route_scores["Comeback"] += 1.2
    if normalized_break_along <= -0.2 and normalized_break_cross >= 0.2:
        route_scores["Comeback"] += 1.0
    if break_style == "rounded":
        route_scores["Comeback"] += 0.3
    if break_field_direction == "reverse":
        route_scores["Curl"] += 0.8
        route_scores["Comeback"] += 0.8
    if signed_turn_angle is not None:
        signed_turn_angle = float(signed_turn_angle)
        if signed_turn_angle <= -90:
            route_scores["Comeback"] += 1.8
            route_scores["Curl"] -= 1.2
        elif signed_turn_angle >= 90:
            route_scores["Curl"] += 1.8
            route_scores["Comeback"] -= 1.2
    if post_break_distance >= 4.5:
        route_scores["Comeback"] += 1.0
    elif post_break_distance >= 3.25:
        route_scores["Comeback"] += 0.45
    if post_break_reaccel is not None and post_break_reaccel >= 1.5:
        route_scores["Comeback"] += 0.8
    if stem_length <= 7.0:
        route_scores["Curl"] += 0.75
        route_scores["Comeback"] -= 0.4

    # Perpendicular-break families.
    perpendicular_signal = 0.0
    if ideal_angle >= 75:
        perpendicular_signal += 1.2
    if 60 <= full_turn_angle <= 120:
        perpendicular_signal += 1.0
    if normalized_break_cross >= 0.55:
        perpendicular_signal += 1.4
    if abs(normalized_break_along) <= 0.5:
        perpendicular_signal += 0.8

    if break_horizontal and abs(break_dx) > abs(break_dy):
        route_scores["Out Route"] += perpendicular_signal + 0.8
    else:
        route_scores["Out Route"] += perpendicular_signal * 0.55

    if (not break_horizontal) and abs(break_dy) > abs(break_dx):
        route_scores["Dig / Square-In"] += perpendicular_signal + 0.8
    else:
        route_scores["Dig / Square-In"] += perpendicular_signal * 0.55

    if break_field_direction == "out":
        route_scores["Out Route"] += 1.6
        route_scores["Dig / Square-In"] -= 0.7
        route_scores["Slant"] -= 0.4
        route_scores["Curl"] -= 1.2
        route_scores["Comeback"] -= 1.0
    elif break_field_direction == "in":
        route_scores["Dig / Square-In"] += 1.6
        route_scores["Slant"] += 0.45
        route_scores["Out Route"] -= 0.7
        route_scores["Curl"] -= 1.6
        route_scores["Comeback"] -= 1.2

    # If the field-direction logic sees a real in/out break and the turn is not
    # actually reversing back on the stem, keep reverse-break families from
    # winning on secondary cues like short post-break travel.
    if break_field_direction in {"in", "out"} and reverse_component > -0.35:
        route_scores["Curl"] -= 1.4
        route_scores["Comeback"] -= 1.0
    if break_field_direction == "in" and normalized_break_cross >= 0.3:
        route_scores["Dig / Square-In"] += 1.0
        route_scores["Slant"] -= 0.5
    if break_field_direction == "in" and deep_break:
        route_scores["Dig / Square-In"] += 1.1
        route_scores["Slant"] -= 0.8
    if break_field_direction == "in" and very_deep_break:
        route_scores["Dig / Square-In"] += 0.9
        route_scores["Curl"] -= 0.6
        route_scores["Comeback"] -= 0.5

    if deep_break:
        route_scores["Out Route"] += 0.55
        route_scores["Dig / Square-In"] += 0.75
        route_scores["Slant"] -= 1.0
    if very_deep_break:
        route_scores["Dig / Square-In"] += 0.55
        route_scores["Slant"] -= 1.2

    # Angled in-breaking family.
    if 20 <= ideal_angle <= 70:
        route_scores["Slant"] += 1.2
    if 20 <= actual_angle <= 70:
        route_scores["Slant"] += 1.0
    if normalized_break_along > -0.15:
        route_scores["Slant"] += 0.7
    if 0.2 <= normalized_break_cross <= 0.85:
        route_scores["Slant"] += 0.5
    if full_turn_angle < 110:
        route_scores["Slant"] += 0.4
    if reverse_component <= -0.1:
        route_scores["Slant"] -= 0.8
    if ideal_angle >= 75:
        route_scores["Slant"] -= 0.6
    if full_turn_angle >= 75:
        route_scores["Slant"] -= 0.8
    if break_field_direction == "in" and full_turn_angle >= 55:
        route_scores["Dig / Square-In"] += 0.9
        route_scores["Slant"] -= 0.6

    best_route, best_score = max(route_scores.items(), key=lambda item: item[1])
    if best_score < 1.6:
        result["route_reason"] = "Not enough route geometry."
        return result

    second_best = max((score for route, score in route_scores.items() if route != best_route), default=0.0)
    margin = best_score - second_best
    result["route_guess"] = best_route
    if best_score >= 3.4 and margin >= 0.8 and cut_confidence == "high":
        result["route_confidence"] = "high"
    elif best_score >= 2.2 and margin >= 0.35:
        result["route_confidence"] = "medium"
    else:
        result["route_confidence"] = "low"

    if best_route == "Curl":
        result["route_reason"] = "The route reverses sharply back toward the stem."
    elif best_route == "Comeback":
        if break_style == "rounded":
    ...
    elif best_route == "Out Route":
        result["route_reason"] = "The break comes off the stem on a near-perpendicular lateral finish."
    elif best_route == "Dig / Square-In":
        result["route_reason"] = "The break comes off the stem toward the middle on a near-perpendicular finish."
    elif best_route == "Slant":
...
def save_speed_graph_plot(df: pd.DataFrame, phase_metrics: dict, out_path: Path):
    if df.empty:
        return

    fig, ax = plt.subplots(figsize=(8, 3.8), facecolor="#0b0f12")
    ax.set_facecolor("#0b0f12")
    time_s = df["time_s"].to_numpy(dtype=float)
    raw_speed = df["speed_mph"].to_numpy(dtype=float)
    display_peak = robust_peak_speed_mph(raw_speed) if len(raw_speed) else 0.0
    speed = smooth_series(smooth_series(raw_speed, window=9), window=13)
    if display_peak > 0:
        speed = np.minimum(speed, display_peak)
    ax.plot(time_s, speed, color="#5dd39e", linewidth=3.0, solid_capstyle="round")
    ax.fill_between(time_s, speed, color="#5dd39e", alpha=0.14)

    cut_frame = phase_metrics.get("cut_frame")
    if cut_frame is not None and "frame" in df.columns:
        hits = df.index[df["frame"] == int(cut_frame)].tolist()
        if hits:
            idx = hits[0]
            ax.axvline(float(time_s[idx]), color="#ef476f", linewidth=2, linestyle="--")
            ax.scatter([float(time_s[idx])], [float(speed[idx])], color="#ef476f", s=42, zorder=5)

    peak_idx = int(np.argmax(speed)) if len(speed) else None
    if peak_idx is not None and len(speed):
        ax.scatter([float(time_s[peak_idx])], [float(speed[peak_idx])], color="#ffd166", s=36, zorder=5)
        ax.text(
            float(time_s[peak_idx]),
            float(speed[peak_idx]) + 0.45,
            f"Peak {display_peak:.1f} mph",
            color="#ffd166",
            fontsize=9,
            ha="center",
        )

    ax.set_title("Speed Through The Route", color="white", fontsize=13)
...
def save_break_snapshot_from_video(video_path: str | None, frame_number: int | None, out_path: Path):
...
def trim_to_manual_window(
    ...
    if start_time_s is not None:
        candidates = np.where(time_s >= float(start_time_s))[0]
    ...
    if end_time_s is not None:
        candidates = np.where(time_s <= float(end_time_s))[0]
...
def build_summary_row(
    ...
    field_scale_factor: float | None = None,
    field_scale_confidence: float | None = None,
) -> pd.DataFrame:
    ...
    row = {
        ...
        "distance_profile": speed_profile.get("distance_profile"),
        "avg_box_height_px": speed_profile.get("avg_box_height_px"),
        "speed_confidence": speed_profile.get("speed_confidence"),
        "speed_confidence_detail": speed_profile.get("speed_confidence_detail"),
        "field_line_scale_factor": field_scale_factor,
        "field_line_scale_confidence": field_scale_confidence,
    }
    row.update(rep_info)
...
def apply_route_display_speed_caps(
    df: pd.DataFrame,
    phase_metrics: dict,
    route_guess: dict,
) -> pd.DataFrame:
    if df.empty or "speed_mph" not in df.columns:
        return df

    route_name = str(route_guess.get("route_guess") or "").strip()
    speed_cap = PREBREAK_ROUTE_SPEED_CAPS_MPH.get(route_name)
    cut_frame = phase_metrics.get("cut_frame")
    if speed_cap is None or cut_frame is None or "frame" not in df.columns:
        return df

    work = df.copy()
    pre_break_mask = work["frame"].to_numpy(dtype=int) <= int(cut_frame)
    if not np.any(pre_break_mask):
        return work

    speed_vals = work["speed_mph"].to_numpy(dtype=float)
    capped_speed = speed_vals.copy()
    capped_speed[pre_break_mask] = np.minimum(capped_speed[pre_break_mask], float(speed_cap))
    work["speed_mph"] = np.round(capped_speed, 3)
    if "speed_yards_per_sec" in work.columns:
        work["speed_yards_per_sec"] = np.round(capped_speed * 1760.0 / 3600.0, 3)
    return work
...
class VisionWRTracker:
    def __init__(
        ...
        self._box_height_history: deque[float] = deque(maxlen=24)
    ...
    def read_frame_at(self, frame_index: int = 0) -> np.ndarray:
    ...
    def read_first_frame(self) -> np.ndarray:
        return self.read_frame_at(0)

    def frame_index_from_time(self, time_s: float | None) -> int:
        if time_s is None or not np.isfinite(time_s):
            return 0
        return max(0, min(int(round(float(time_s) * self.fps)), max(0, self.total_frames - 1)))
    ...
    def select_target_from_point(
        ...
        if containing:
            ...
            raise RuntimeError("No detected player was under the selected point. Try a nearby frame or choose a clip where the receiver is clearly detected.")
    ...
    def track(
        ...
        start_frame_index = max(0, min(int(start_frame_index), max(0, self.total_frames - 1)))
        self.frame_offset = start_frame_index
        ...
        first_frame = self.read_frame_at(start_frame_index)
        ...
        self._world_history: deque[tuple[float, float]] = deque(maxlen=max(SPEED_MEDIAN_WINDOW, SPEED_LOOKBACK_FRAMES + 2))
        self._box_height_history.clear()
    ...
    def _compute_metrics(
        ...
        if self.use_pose and box_height_px is not None and box_height_px > 1.0:
            self._box_height_history.append(float(box_height_px))
            heights = np.array(self._box_height_history, dtype=float)
            heights = heights[np.isfinite(heights) & (heights > 1.0)]
            if len(heights) >= 4:
                recent_baseline = float(np.percentile(heights, 75))
                running_max = float(np.max(heights))
                clip_median = float(np.median(heights))
                recent_ratio = float(box_height_px) / max(recent_baseline, 1e-6)
                global_ratio = float(box_height_px) / max(running_max, 1e-6)
                ratio = min(recent_ratio, global_ratio)
                median_ratio = float(box_height_px) / max(clip_median, 1e-6)
                live_scale = 1.0
                if median_ratio < 0.96:
                    live_scale *= min(1.1, max(1.0, median_ratio ** -0.16))
                elif median_ratio > 1.06:
                    live_scale *= max(0.84, median_ratio ** -0.35)

                if ratio < 0.9:
                    live_scale *= max(0.82, ratio ** 0.35)
                elif ratio > 1.02:
                    live_scale *= max(0.88, ratio ** -0.18)

                filtered_speed_yps *= live_scale
    ...
    def _draw_frame_overlay(
        ...
        display_speed_mph, display_accel_mph_per_sec = preview_speed_display_values(
            float(metrics["speed_mph"]),
            float(metrics["accel_mph_per_sec"]),
        ...
        lines = [
            ...
            f"Live speed est.: {display_speed_mph:.2f} mph",
            f"Live accel est.: {display_accel_mph_per_sec:.2f} mph/s",
        ]
...
def save_outputs(
    ...
    manual_start_time_s: float | None = None,
    manual_end_time_s: float | None = None,
    cut_frame_override: int | None = None,
    include_pose_metrics: bool = True,
    ...
    field_scale_factor = resolve_field_scale_factor(df, field_scale_factor, field_scale_confidence)
    df = apply_field_scale_correction(df, field_scale_factor)
    df = recompute_frame_kinematics(df)
    df = apply_speed_adjustment(df, calibration_confidence, analysis_mode, field_scale_factor=field_scale_factor)
    speed_profile = infer_speed_profile(df, calibration_confidence, analysis_mode, field_scale_factor=field_scale_factor)
    ...
    speed_graph_path = output_stem.with_name(output_stem.name + "_speed_graph.png")
    break_snapshot_path = output_stem.with_name(output_stem.name + "_break_snapshot.jpg")
    df.to_csv(csv_path, index=False)

    if manual_start_frame is not None or manual_end_frame is not None or manual_start_time_s is not None or manual_end_time_s is not None:
        rep_df, rep_info = trim_to_manual_window(
            df,
            manual_start_frame,
            manual_end_frame,
            manual_start_time_s,
    ...
    summary_display_df = apply_route_display_speed_caps(summary_df_source, phase_metrics, route_guess)
    peak_speed = robust_peak_speed_mph(summary_display_df["speed_mph"].to_numpy(dtype=float)) if not summary_display_df.empty else 0.0
    instant_peak_speed = float(summary_display_df["speed_mph"].max()) if not summary_display_df.empty else 0.0
    avg_speed = float(summary_display_df["speed_mph"].mean()) if not summary_display_df.empty else 0.0
    peak_accel = float(summary_display_df["accel_mph_per_sec"].max()) if not summary_display_df.empty else 0.0
    clean_df = build_clean_metrics_csv(df)
    pose_df = build_pose_points_csv(df)
    rep_clean_df = build_clean_metrics_csv(summary_display_df)
    summary_df = build_summary_row(
        ...
        field_scale_confidence=field_scale_confidence,
    )
    clean_df.to_csv(clean_csv_path, index=False)
    pose_df.to_csv(pose_csv_path, index=False)
    rep_clean_df.to_csv(rep_clean_csv_path, index=False)
    summary_df.to_csv(summary_csv_path, index=False)
    save_speed_graph_plot(summary_display_df, phase_metrics, speed_graph_path)
    save_break_snapshot_from_video(overlay_video_path, phase_metrics.get("cut_frame"), break_snapshot_path)
    ...
    summary_lines.append(f"distance_profile={speed_profile.get('distance_profile')}")
    summary_lines.append(f"avg_box_height_px={speed_profile.get('avg_box_height_px')}")
    summary_lines.append(f"speed_confidence={speed_profile.get('speed_confidence')}")
    summary_lines.append(f"speed_confidence_detail={speed_profile.get('speed_confidence_detail')}")
    summary_lines.append(f"field_line_scale_factor={field_scale_factor}")
    summary_lines.append(f"field_line_scale_confidence={field_scale_confidence}")
    summary_lines.append(f"prebreak_speed_cap_mph={PREBREAK_ROUTE_SPEED_CAPS_MPH.get(route_guess.get('route_guess'))}")
    ...
    summary_lines.append(f"speed_graph={speed_graph_path}")
    summary_lines.append(f"break_snapshot={break_snapshot_path}")
    ...
    save_route_debug_plot(summary_display_df, phase_metrics, route_guess, debug_plot_path)
    ...
    print(f"Saved speed graph: {speed_graph_path}")
    print(f"Saved break snapshot: {break_snapshot_path}")
...
def run(
    ...
    try:
        selection_start_frame = (
            ...
            else tracker.frame_index_from_time(start_time)
        )
        first_frame = tracker.read_frame_at(selection_start_frame)
        field_line_scale_factor = None
        field_line_scale_confidence = None
        ...
        if mode == "side-view" and homography is not None:
            field_line_scale_factor, field_line_scale_confidence, field_line_info = estimate_sideview_field_line_scale(first_frame, homography)
            if field_line_scale_factor is not None:
                print(
                    "Field-line scale estimate "
                    f"(factor={field_line_scale_factor:.3f}, confidence={field_line_scale_confidence:.2f}, "
                    f"clusters={field_line_info.get('line_clusters')}, median_gap={field_line_info.get('median_gap_yards')})"
                )
        ...
        df = tracker.track(
            ...
            progress_image_path=progress_image_path,
            start_frame_index=selection_start_frame,
        )
        ...
        save_outputs(
            ...
            manual_start_time_s=start_time,
            manual_end_time_s=end_time,
            cut_frame_override=cut_frame,
            include_pose_metrics=tracker.use_pose,
            analysis_mode=mode,
            overlay_video_path=overlay_path,
            field_scale_factor=field_line_scale_factor if (field_line_scale_confidence or 0.0) >= 0.25 else None,
            field_scale_confidence=field_line_scale_confidence,
        )
    finally:
        tracker.release()
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--start-time", type=float, default=None, help="Optional manual rep start time in seconds for trimming outputs")
    parser.add_argument("--end-time", type=float, default=None, help="Optional manual rep end time in seconds for trimming outputs")
